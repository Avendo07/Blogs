+++
title = 'Building and hosting your own blog site'
...
As it turns out that Github also provides a static website hosting called [Github Pages](https://pages.github.com/). This is a free of cost service and was a perfect service available to everyone with a github account, thus I chose this.

Now it was time to look for a framework to generate the static website. There are multiple JS frameworks which could do the trick such as NextJS or NuxtJS, but I went for Hugo as it is was the one most people suggested due to significantly faster build times and had more than enough templates to get started with a basic setup done pretty quickly. Also, it's based on golang which I like, so that's a plus.
...
These themes can be added to a project generally in three ways:
- Initialize the site as a hugo module and add theme as a hugo module. Edit the hugo.yaml configuration file to include the theme as module:
    ```yaml
    ...
    module:
      imports:
        ...
        - path : "github.com/gohugoio/hugo-mod-bootstrap-scss/v5"
          disable : false 
    ```
- Using git submodules(Most recommended)
  - Initialise the git repository using `git init`
  - Use the command `git submodule add <hugo_theme_url> themes/<theme_name>`
  - Edit the hugo.yaml configuration file to include the theme
  ```yaml
    baseURL : 'http://example.org/'
    languageCode : 'en-us'
    title : 'My New Hugo Site'
    module:
      imports:
        ...
        - path : "github.com/gohugoio/hugo-mod-bootstrap-scss/v5"
          disable : false 
    ```
- By cloning the themes locally
  - This is generally used for local theme development
  - This can be done by cloning the theme repository to 

Themes generally have their own exposed settings that you can customise, it can be configured in the existing `config.yaml` file or can be setup in different files using a tree [file structure](https://gohugo.io/getting-started/configuration/#recursive-parsing).
You can refer to the examples mentioned in the repositories of the themes, it can be a good starting place.
...
- Go to the pages section and enable github pages section
- Set the source to Github Actions to publish the site using a github actions pipeline
- You can also host this website on custom domains by entering the domain name and dns a-records in the required section

You could optionally use one of the suggested workflows to get the build pipeline set up if you are using one of the other frameworks. Follow along for more details on the deployment step of the pipeline
...
## The final workflow
The deployment pipeline is set up to deploy and host the website on Github Pages, let's discuss how the workflow looks like after this initial setup.

### Adding new blogs
For adding a new blog to the site, we need to create a new markdown file in the `content/blogs`(specific to the current setup) directory, this would add a blog to the blogs section of the website. This can also be done using `hugo new content <filepath/filename.md>` to create a new post with boilerplate syntax picked up from the archetypes directory. 

To add images to the site, it can be added to the `static` directory and can be referenced in the markdown documents using `/<image-name>.png`.

This is the setup that I chose to go with.
- Chose this theme as met all the requirements I had - `Hugo Profile Theme`[^2]
- You can take a look at the config file for the hugo site [here](https://github.com/Avendo07/hugo-profile.git)
...
You can look at this [documentation](https://gohugo.io/content-management/front-matter/) to see more details about the fields that go in the frontmatter. These will only be analysed when a new build is created though, if this is supposed to be analysed daily, set the pipeline to run daily at some point of time.

After we make a commit to the repository and push it to the VCS(Github here), the pipeline should get triggered and create a build of the current content and add it to the outputs directory. The contents of the outputs directory is published on the website.
You can check the repository here

`Note:`
I am using a [fork](https://github.com/Avendo07/hugo-profile) of the `hugo profile theme`[^2] and made some minor changes
...
[^2]: [Hugo Profile theme](https://github.com/gurusabarish/hugo-profile)